...
class Mu256ControlBoard(QMainWindow) : 
    ...
    def StartStop(self):
        if not self.Mu.running:
            ...
            # Create the Qt timer and connect the PlotOnTheFly method to the timeout signal
            self.timer = QTimer()
            self.timer.timeout.connect( lambda: self.PlotOnTheFly() )
            
            # Start the Mµ256 acquisition
            self.LaunchMu()
